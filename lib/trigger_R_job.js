...
var env = process.env
if(!env.CALVAD_OUTPUT_PATH) env.CALVAD_OUTPUT_PATH = outputdir
if(!env.CALVAD_VDSWIM_IMPUTE_MAXITER) env.CALVAD_VDSWIM_IMPUTE_MAXITER = 200
if(!env.CALVAD_VDS_PATH) env.CALVAD_VDS_PATH = '/data/backup/pems'
...
function trigger_R_job(vds_id,wim_sites,task,done){
    ...
    wim_sites.forEach(function(v){
        if(v.dist < mindist){
            wim_pairs_string.push(+v.vds_id,v.site_no,v.direction.substring(0,1).toUpperCase())
        }
        return null
    })
