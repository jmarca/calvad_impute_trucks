/**
 * @fileOverview
 * @name trigger_wim_impute_v_o.js
 * @author James E. Marca <james@activimetrics.com>
 * @license GPL v2
 *
 * This program will call R to impute the V O estimates for all WIM
 * sites.  It does not check for those already completed, and so does
 * not need a "redo" parameter in config.json like the vds truck
 * imputation script does.
 *
 * config.json should have the usual PSQL and CouchDB connection
 * information, and this program will look for calvad.years (either a
 * single year or an array of years) and will use that.  If years is
 * not defined in the config.json, then this program will not run.
 */
/*global console */
...
mainQ.await(function(e,config){
    ...
    if (years === undefined){
        throw new Error('please define a year or years in config.json, using calvad.years parameter entry.')
    }
