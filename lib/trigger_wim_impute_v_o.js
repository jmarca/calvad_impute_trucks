...
if(argv.rscript === undefined){
    rscript = path.normalize(rootdir+'/../R/wim.n.o.impute.R')
}else{
    ...
    seQ.await(function(e,wim_sites){
        ...
        create_dist_table(config,function(e,table){
            ...
            wim_sites.forEach(function(doc){
                wim_site_docs[doc.id]=doc.doc[year]
                var property_key = Object.keys(doc.properties).sort().pop()
                wim_site_docs[doc.id].properties=doc.properties[property_key]
            })
            ...
            _.each(table,function(wim_dists,cdb_wimid){
                ...
                wim_dists.forEach(function(row){
                    ...
                    var other_lanes =
                            wim_site_docs[paired_cdbwimid].properies.lanes
                ...
                if(wim_pairs.length > 3){
                    ...
                    wim_pairs = wim_pairs.slice(0,3) // keep 0,1,2
